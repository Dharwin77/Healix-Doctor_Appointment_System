...
import { Calendar, Clock, MapPin, Star, Search, User, X, MessageCircle } from "lucide-react";
...
import OfflineAppointmentModal from "./OfflineAppointmentModal";
import { useNavigate } from "react-router-dom";

const API_URL = import.meta.env.VITE_API_URL;

const BookAppointment = ({ user, onStartChat }) => {
  ...
  // New state for appointment type selection
  const [showAppointmentTypeModal, setShowAppointmentTypeModal] = useState(false);
  const [showOfflineAppointmentModal, setShowOfflineAppointmentModal] = useState(false);
  ...
  const handleAppointmentTypeSelect = (appointmentType) => {
    ...
    if (appointmentType === 'online') {
      // Show the existing online appointment modal (current video integration)
      setShowBookingModal(true);
    } else if (appointmentType === 'offline') {
      // Show the new offline appointment form
      setShowOfflineAppointmentModal(true);
    }
  };

  const handleChatWithDoctor = (doctor) => {
    if (onStartChat) {
      // Convert doctor object to match expected format
      const doctorForChat = {
        _id: doctor._id,
  ...
  return (
    <div className="max-w-7xl w-full mx-auto px-4 py-8">
      ...
      <AppointmentTypeModal
        open={showAppointmentTypeModal}
        onClose={() => setShowAppointmentTypeModal(false)}
        doctor={selectedDoctor}
        onSelectType={handleAppointmentTypeSelect}
      />

      {/* Offline Appointment Modal */}
      <OfflineAppointmentModal
        open={showOfflineAppointmentModal}
        onClose={() => setShowOfflineAppointmentModal(false)}
        doctor={selectedDoctor}
        user={user}
      />
    </div>
