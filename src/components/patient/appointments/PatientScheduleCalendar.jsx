      ...
      if (data.success) {
        ...
        setEvents(
          data.appointments.map((apt) => {
            const appointmentType = apt.appointmentType === 'online' ? 'Online' : 
                                  apt.appointmentType === 'offline' ? 'Offline' :
                                  'Online'; // Default to Online for consultation, follow-up, emergency types
            const doctorName = `Dr. ${apt.doctorId?.firstName || ""} ${apt.doctorId?.lastName || ""}`;
            return {
              id: apt._id,
              title: `${appointmentType} with ${doctorName}`,
              start: new Date(
